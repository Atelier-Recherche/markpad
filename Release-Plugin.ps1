...
.DESCRIPTION
  ...
  6) Plugin seul à la racine (pas monorepo)
     Mettre $PluginSubdir = '.' ; $CoreWorkspace = $null ; $PluginWorkspace = $null ;
     build via npm run build dans $PluginSubdir au lieu de -w.

  Markpad : mono-repo avec $PluginSubdir = 'plugin', $PluginWorkspace = '@markpad/plugin'.
...
$GhActionsUrl = 'https://github.com/Atelier-Recherche/markpad/actions' # $null pour ne rien afficher
